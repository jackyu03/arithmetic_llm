"""Contrastive learning for arithmetic LLM: correct vs incorrect reasoning chains.

Goal (proposal): Transcend pattern matching by training on pairs of correct and
incorrect solution paths, so the model learns to distinguish valid logic from
common arithmetic fallacies and internalize mathematical rules rather than
memorize successful templates.

Mechanism:
- Pairs: each sample has (correct completion, wrong completion) where the wrong
  solution is generated by make_wrong_solution to mimic realistic fallacies
  (e.g. tens-digit error in one step then propagated, or dropped expression subtree).
- Loss: margin over log-probability—correct completion preferred over wrong
  (softplus(-(L_correct - L_wrong) / temperature)), so the model implicitly
  "critiques" by assigning lower likelihood to invalid reasoning.
- Result-token contrastive (default): margin only at "Step ... = <result>" and
  "Final Result: <answer>" positions, so the signal focuses on arithmetic
  decisions rather than being diluted by sequence length.
- Optional completion-level contrastive: margin over full completion logP.
"""
...
def make_wrong_solution(
    ...
    """Create a wrong solution (arithmetic fallacy) for contrastive negatives.

    Three types, all with one error then propagated to final:
    - Tens-digit: pick one step with result >= 10, flip tens digit; replace that number
      everywhere after; recompute later steps and Final Result.
    - Delta (any step): pick any step, set result to result + delta; same propagate + recompute.
    - Drop subtree: drop one top-level subtree in an "Expression now" line; evaluate the
      remaining expression and set Final Result (and last Expression now) to that value.
    ...
    Args:
        ...
        allow_drop_subtree: If False, never use drop-subtree type (only step wrong + propagate)
    ...
    # 0 = tens-digit + propagate, 1 = any-step delta + propagate, 2 = drop subtree + eval to final
    if allow_drop_subtree:
        corruption_type = rng.choices([0, 1, 2], weights=[0.50, 0.42, 0.08], k=1)[0]
    else:
        corruption_type = rng.choices([0, 1], weights=[0.55, 0.45], k=1)[0]

    out = solution

    if corruption_type == 2:
        # Drop one subtree in an "Expression now: EXPR" line; eval remaining expression → final
        expr_now_matches = list(_EXPR_NOW_PATTERN.finditer(out))
        wrong_final: Optional[int] = None
        if expr_now_matches:
            ...
            if dropped is not None:
                ...
                res = eval_expression(dropped)
                if res.get("answer") != "ERROR":
                    try:
                        wrong_final = int(res["answer"])
                    except (TypeError, ValueError):
                        pass
        if wrong_final is None:
            wrong_final = correct_answer + delta
        wrong_str = str(wrong_final)
        ...
        if expr_single:
            ...
            out = out[: last_expr.start(2)] + wrong_str + out[last_expr.end(2) :]
        return out

    # Type 0 (tens-digit) or 1 (delta): one step wrong, then propagate + recompute to final
    ...
    if corruption_type == 0:
        # Tens-digit wrong: only steps with result >= 10
    ...
    if corruption_type == 1 or (corruption_type == 0 and corrupt_idx is None):
        # Any step wrong by delta + propagate (first-class type or fallback when no tens-digit)
